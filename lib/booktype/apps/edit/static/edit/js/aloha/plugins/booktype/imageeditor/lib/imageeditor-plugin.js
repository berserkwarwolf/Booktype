        ...
        PubSub.sub('bk-image-editor.start', function (message) {
          ...
          if (
            ...
            if (jQuery(message.frame).next().hasClass('image')) {
              BkImageEditorInstance.FRAME_MAX_WIDTH = BkImageEditorInstance.editorWidth - jQuery(message.frame).next().width() - 2;
            } else {
              BkImageEditorInstance.FRAME_MAX_WIDTH = BkImageEditorInstance.editorWidth - jQuery(message.frame).prev().width() - 2;
            }
          } else if (layout === 'image-layout-2images_2captions_bottom') {
            if (jQuery(message.frame).parent().next().hasClass('wrap')) {
              BkImageEditorInstance.FRAME_MAX_WIDTH = BkImageEditorInstance.editorWidth - jQuery(message.frame).parent().next().find('.image').width() - 2;
            } else {
              BkImageEditorInstance.FRAME_MAX_WIDTH = BkImageEditorInstance.editorWidth - jQuery(message.frame).parent().prev().find('.image').width() - 2;
            }
          } else if (
